...
dhtmlXGridObject.prototype.filterBy = function (a, b, c) {
    ...
    if (this.isTreeGrid()) {
        return this.filterTreeBy(a, b, c);
    }
    ...
    if (this.rowsBuffer.length) {
        ...
        this.dma(!1);
        this.pagingOn && this.rowsBuffer.length / this.rowsBufferOutSize < this.currentPage - 1 && this.changePage(0);
        this._reset_view();
...
dhtmlXGridObject.prototype._filterA = function (a, b) {
    if (b != "") {
        var c = !0;
        typeof b == "function" ? c = !1 :
        b = (b || "").toString().toLowerCase();
        if (this.rowsBuffer.length) {
            for (var d = this.rowsBuffer.length - 1; d >= 0; d--) {                
                (c ? this._get_cell_value(this.rowsBuffer[d], a).toString().toLowerCase().indexOf(b) == -1 : !b.call(this, this._get_cell_value(this.rowsBuffer[d], a), this.rowsBuffer[d].idd)) && this.rowsBuffer.splice(d, 1);
            }
        }        
    }
};
...
!1), i; for (i in c) if (c[i] === !0) { if (h) if (h.get && h.get(i)) i = h.get(i); else if (h.getOption && h.getOption(i)) i = h.getOption(i).text; d.push(i) } return d.sort()
}; dhtmlXGridObject.prototype._build_m_order = function () { if (this._c_order) { this._m_order = []; for (var a = 0; a < this._c_order.length; a++) this._m_order[this._c_order[a]] = a } };
...
dhtmlXGridObject.prototype.filterByAll = function ()
{
    ...
    for (var c = 0; c < this.filters.length; c++) {
        if (!(c >= this._cCount)) {
            ...
            var f;
            if (typeof e != "function" && (f = this.combos[d] || (this._col_combos ? this._col_combos[d] : !1)))
                f.values ? (d = f.values._dhx_find(e), e = d == -1 ? e : f.keys[d]) : f.getOptionByLabel && (e = f.getOptionByLabel(e) ? f.getOptionByLabel(e).value : e); a.push(e)
        }
    }
...
dhtmlXGridObject.prototype.makeFilter = function (a, b) {
    if (!this.filters) this.filters = []; typeof a != "object" && (a = document.getElementById(a));
    if (a) {
        var c = this;
        ...
        if (a.tagName == "SELECT") {
            ...
            a.onchange = function () { c.filterByAll() };
            if (_isIE) a.style.marginTop = "1px";
            this.attachEvent("onEditCell", function (c, d, g) {
                this._build_m_order(); c == 2 && this.filters && (this._m_order ? g == this._m_order[b] : g == b) && this._loadSelectOptins(a, b);
                return !0
            })
        }
        else if (a.tagName == "INPUT") {
            this.filters.push([a, b]), a.old_value = a.value = "", a.onkeydown = function () {
                this._timer && window.clearTimeout(this._timer);
                this._timer = window.setTimeout(function () { if (a.value != a.old_value) c.filterByAll(), a.old_value = a.value }, 500)
            };
        }
            ...
            d.filterSelf = function () {
                this.getSelectedIndex() == 0 && this.setComboText("");
            ...
            d.attachEvent("onChange", function () { a.value = d.getSelectedValue(); if (a.value === null) a.value = ""; c.filterByAll() })
        }
...
dhtmlXGridObject.prototype._loadSelectOptins = function (a, b) {
    ...
    for (var e = this._filter_tr ? this._filter_tr[b] : null, f = 0; f < c.length; f++) {        
        //如果有^截取第一个之前
        var ii = c[f].indexOf('^');
        if (ii > -1) {
            c[f] = c[f].substring(0, ii);
        }

        a.options[a.options.length] = new Option(e ? e(c[f]) : c[f], c[f]);
    }
    a.value = d;
};

dhtmlXGridObject.prototype.setSelectFilterLabel = function (a, b) { if (!this._filter_tr) this._filter_tr = []; this._filter_tr[a] = b };
